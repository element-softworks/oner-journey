  ...
  return (
    <div className="w-full h-screen flex flex-col bg-gray-50">
      <div className="flex flex-col items-center pt-8 px-6">
        ...
        />
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Your Choice</h1>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <div className="w-24 h-24 rounded-lg overflow-hidden bg-gray-100">
                <img 
                  src={selectedTopItem.colorImages?.[selectedTopColor] || selectedTopItem.image}
                  alt={selectedTopItem.name}
                  className="w-full h-full object-cover"
                />
              </div>
              <div>
                <h3 className="font-semibold text-gray-900">{selectedTopItem.name}</h3>
                <p className="text-sm text-gray-600">
                  {selectedTopItem.colors.find(c => c.value === selectedTopColor)?.name}
                </p>
                <p className="text-sm font-medium text-gray-900">{selectedTopItem.price}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-semibold text-gray-900">{selectedBottomItem.name}</h3>
                <p className="text-sm text-gray-600">True Blue</p>
                <p className="text-sm font-medium text-gray-900">{selectedBottomItem.price}</p>
              </div>
            </div>
          </div>
      ...
      <div className="p-6 space-y-4">
        <Button
          onClick={handleEdit}
          variant="outline"
          className="w-full h-14 border-gray-200 hover:bg-gray-50 group"
        >
